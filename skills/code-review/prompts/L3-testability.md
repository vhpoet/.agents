...
**Bar for reporting**: Report structural obstacles, not missing tests. Each finding must name the specific restructuring that creates the seam — "extract the decision into a pure function", "accept the clock as a parameter" — not "make this more testable". An unenforced invariant is a structural finding, not a coverage finding — report it when a type, a boot assert, or a source-scanning test could hold it, and name that mechanism in the fix.
